...
public class LogRecordIterator implements Iterator<LogRecord> {
   ...
   private LogIterator forwardIterator = (LogIterator) SimpleDB.logMgr().forwardIterator();
   
   public LogRecordIterator(LogRecordIterator reverseIterator) {
	   iter = SimpleDB.logMgr().forwardIterator(reverseIterator.iter());
   }
   
   public LogRecordIterator() {
	  iter = SimpleDB.logMgr().iterator();
   }
   ...
   public boolean hasNextForward() {
      return forwardIterator.hasNextForward();
   }
   ...
   public LogRecord nextForward() {
      BasicLogRecord rec = forwardIterator.next();
      int op = rec.nextInt();
      switch (op) {
         case CHECKPOINT:
            return new CheckpointRecord(rec);
