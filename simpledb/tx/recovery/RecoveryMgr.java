...
public class RecoveryMgr {
	...
	public int setInt(Buffer buff, int offset, int newval) {
		...
		if (isTempBlock(blk))
			...
			return new SetIntRecord(txnum, blk, offset, oldval, newval)
					.writeToLog();
	}
	...
	public int setString(Buffer buff, int offset, String newval) {
		...
		if (isTempBlock(blk))
			...
			return new SetStringRecord(txnum, blk, offset, oldval, newval)
					.writeToLog();
	}
	...
	private void doRecover() {
      ...
      LogRecordIterator iter_redo = new LogRecordIterator();
    	  LogRecord rec = iter_redo.nextForward();
    	  if (finishedTxs.contains(rec.txNumber())) {
    		  rec.redo(txnum);
    		  System.out.println("Record object after redo: " + rec);
    	  }
      LogRecordIterator iter_undo = new LogRecordIterator();
      while (iter_undo.hasNext()) {
         rec = iter_undo.next();
         ...
         if (rec.op() == COMMIT || rec.op() == ROLLBACK)
            ...
            rec.undo(txnum);
        	System.out.println("Record object after undo/redo: " + rec);
      }
