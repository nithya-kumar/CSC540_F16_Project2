...
public class LogMgr implements Iterable<BasicLogRecord> {
   ...
   /**
    * Returns an iterator for the log records,
    * which will return the first record.
    * @see java.lang.Iterable#iterator()
    */
   public synchronized Iterator<BasicLogRecord> forwardIterator() {
	      flush();
	      Block block = new Block(logfile, 0);
	      return new LogIterator(block, 1);
   }
   
   /**
    * Returns an iterator for the log records,
    * which will be returned in forward order starting with the backward iterator.
    * @see java.lang.Iterable#iterator()
    */
   public synchronized Iterator<BasicLogRecord> forwardIterator(LogIterator backwardIterator) {
      flush();
      return new LogIterator(backwardIterator);
   }
   
   public synchronized Iterator<BasicLogRecord> iterator(Block block) {
	   flush();
	   return new LogIterator(block, 1);
   }
   /**
